...
7. [Phase 2 — Full-Weight Fine-Tuning (SFT)](#7-phase-2--full-weight-fine-tuning-sft)
...
- Approximately 250 problems

**Total target dataset size:** 1,450 problems (sufficient for robust 7b model fine-tuning)
...
Expected pass rate: ~78% (rejecting ~22% of generated samples that failed execution checks).
...
For 1,450 problems at ~2 requests each (initial + retry for failed quality checks):
...
## 7. Phase 2 — Full-Weight Fine-Tuning (SFT)

### 7.1 Why Full-Weight SFT

We fine-tuned Qwen2.5-Coder-7B-Instruct via full-weight SFT to establish a robust supervised baseline before reinforcement alignment. While parameter-efficient methods like LoRA save memory, full-weight tuning provides the strongest possible foundation for the subsequent RL steps.
...
We ablated the four reward functions individually to isolate their contributions. We found that the language-consistency reward contributed most to Arabic fluency, while the correctness reward drove the largest accuracy gains, informing our final reward-weighting decisions.

| Experiment | Format % | Code Correct % | Arabic Ratio | Reasoning Score (avg) |
|---|---|---|---|---|
| Full GRPO | 95% | 23% | 0.82 | 7.00 |
| No Format Reward | 71% | 18% | 0.79 | 6.40 |
| No Correctness Reward | 92% | 15% | 0.83 | 6.50 |
| No Language Reward | 94% | 21% | 0.41 | 6.80 |
| No Logic Reward | 93% | 22% | 0.80 | 6.00 |
